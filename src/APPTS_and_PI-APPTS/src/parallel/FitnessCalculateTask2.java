...
// Reduced Fork/Join move evaluation used after the candidate array is already free
// of forbidden-tuple violations.
public class FitnessCalculateTask2 extends RecursiveTask<Result> {
    ...
    public FitnessCalculateTask2(int row, int column, int newValue, ArrayList<Tuple> newMfts,
            ArrayList<CombinationPosition> unCoveredList, ArrayList<CombinationPosition> onceCoveredList,
            Tuple newCheckedTuple, int start, int end) {
        this.row = row;
        this.column = column;
        ...
        this.start = start;
        this.end = end;
        int len1 = this.newMfts.size();
        int len2 = this.unCoveredList.size();
        int len3 = this.onceCoveredList.size();
        this.THRESHOLD = Math.max(10, (len1 + len2 + len3) / (Runtime.getRuntime().availableProcessors() * 4));
    }
    ...
    protected Result compute() {
        if (end - start <= THRESHOLD) {
            ...
            for (int i = start; i < end; i++) {
                if (i < len1)
                    fitness_mft += addedMft(i );
                else if (i < len1 + len2)
                    fitness_comb -= deletedUncoveredComb(i - len1);
                else
                    fitness_comb += addedUncoveredComb(i - len1 - len2);
            }
            return new Result(fitness_mft, fitness_comb);
        } else {

            int mid = (start + end) / 2;
            FitnessCalculateTask2 leftTask = new FitnessCalculateTask2(row, column, newValue, newMfts,
                    unCoveredList, onceCoveredList,newCheckedTuple, start, mid);
            FitnessCalculateTask2 rightTask = new FitnessCalculateTask2(row, column, newValue, newMfts,
                    unCoveredList, onceCoveredList,newCheckedTuple, mid, end);
