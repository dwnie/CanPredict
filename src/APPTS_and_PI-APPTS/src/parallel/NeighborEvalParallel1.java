...
public class NeighborEvalParallel1 implements Serializable,
        NeighborEval {
    ...
    public NeighborEvalParallel1(int row, int column, int newValue, Element[] neighbors, int no) {
        this.row = row;
        this.column = column;
        ...
        oldMfts = Main.checker.getMFTbyParam(column, Main.coverageArray[row][column]);
        ...
        int[] oldCheckedArray = new int[2 * Main.paraNum];
        int[] newCheckedArray = new int[2 * Main.paraNum];
        for (int k = 0; k < Main.paraNum; k++) {
            oldCheckedArray[2 * k] = k;
            newCheckedArray[2 * k] = k;
            oldCheckedArray[2 * k + 1] = Main.coverageArray[row][k];
            newCheckedArray[2 * k + 1] = Main.coverageArray[row][k];
        }
        newCheckedArray[2 * column + 1] = newValue;
        oldCheckedTuple = new Tuple(oldCheckedArray);
        newCheckedTuple = new Tuple(newCheckedArray);
    }

    public void caculEffcAndTabu() {

        int len = oldMfts.size() + newMfts.size() + unCoveredList.size() + onceCoveredList.size();

        ForkJoinPool pool = new ForkJoinPool();

        Result result = pool.invoke(
                new FitnessCalculateTask(row, column, newValue, oldMfts, newMfts, unCoveredList, onceCoveredList,
                        oldCheckedTuple, newCheckedTuple, 0, len));
